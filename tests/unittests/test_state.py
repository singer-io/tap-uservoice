...
class TestIncorporate(unittest.TestCase):
    ...
    def test_incorporate_new_bookmark(self):
        ...
        state = {}
        new_state = incorporate(state, 'categories', 'updated_at', '2024-01-15T10:30:00Z')
        self.assertIn('bookmarks', new_state)
        self.assertIn('categories', new_state['bookmarks'])
        self.assertEqual(new_state['bookmarks']['categories']['field'], 'updated_at')
        self.assertIsNotNone(new_state['bookmarks']['categories']['last_record'])
    ...
    def test_incorporate_updates_when_newer(self):
        ...
        new_state = incorporate(state, 'categories', 'updated_at', '2024-06-15T12:00:00Z')
        self.assertEqual(
            new_state['bookmarks']['categories']['last_record'],
    ...
    def test_incorporate_does_not_update_when_older(self):
        ...
        state = {
            'bookmarks': {
                'categories': {
                    'field': 'updated_at',
                    'last_record': '2024-06-15T12:00:00Z'
                }
            }
        }
        new_state = incorporate(state, 'categories', 'updated_at', '2024-01-01T00:00:00Z')
        self.assertEqual(
            new_state['bookmarks']['categories']['last_record'],
            '2024-06-15T12:00:00Z'
        )
    ...
    def test_incorporate_none_value_returns_state_unchanged(self):
        ...
        state = {'bookmarks': {}}
        new_state = incorporate(state, 'categories', 'updated_at', None)
...
class TestGetLastRecordValueForTable(unittest.TestCase):
    ...
    def test_returns_parsed_date(self):
        ...
        result = get_last_record_value_for_table(state, 'categories')
...
class TestSaveState(unittest.TestCase):
    ...
    @unittest.mock.patch('tap_uservoice.state.singer.write_state')
    def test_save_state_calls_write_state(self, mock_write_state):
        """Test that save_state calls singer.write_state."""
        state = {'bookmarks': {'categories': {'last_record': '2024-01-01'}}}
        save_state(state)
